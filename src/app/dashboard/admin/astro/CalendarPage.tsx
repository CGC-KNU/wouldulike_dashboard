 ...
 *
 * 세 곳에서 날짜를 모은다 — 파트너 후보(컨택·미팅·기한), 파트너 매장(견적서·계약서 회수·체결·시작·종료·입금 예정),
 * 세금계산서(입금 완료·발행·품의·승인). 날짜 칸이 있으면 올라온다 (민열님 0914).
...
export default function CalendarPage({ actor, onGo }: { actor: string; onGo: (target: string) => void }) {
  ...
  const loading = !stores || !leads || !invoices;
  ...
  return (
    <>
      ...
      <div className="sat-stagger grid grid-cols-2 lg:grid-cols-4 gap-2.5 mb-5">
        ...
        <Kpi label="계약 종료" value={loading ? "-" : n("contract_end")} tone={n("contract_end") ? "alert" : "plain"} hint="갱신 이야기를 꺼낼 곳" onClick={() => onGo("astro-ops")} />
      </div>
      ...
      <p className="text-[12px] text-gray-500 mt-3 leading-relaxed">
        <span className="font-semibold text-gray-700">날짜가 적힌 것은 전부 올라옵니다</span> — 후보의 컨택·미팅·기한, 매장의 견적서 발송·계약서 회수·계약 체결·시작·종료, 청구의 입금 예정·입금 완료·계산서 발행까지.
