...
export default function ProbeHome({ onGo }: { onGo: (tab: string) => void }) {
  ...
  const [ins, setIns] = useState<{ insights?: { store: string; topic: string; checkpoint: string; age_days: number | null }[]; papillon_reachable?: boolean } | null>(null);
  ...
  const due = (ins?.insights ?? []).filter((i) => ["D2", "D7", "D14"].includes(i.checkpoint));
  ...
  return (
    <>
      <PageHeader
        title="데이터 홈"
        description="보고할 차례인 홍보 게시물, 추첨 응모풀, 어긋난 데이터. 숫자를 누르면 해당 화면으로 갑니다."
      ...
      <div className="sat-stagger grid grid-cols-2 lg:grid-cols-4 gap-2.5 mb-5">
        <Kpi label="보고할 차례" value={!ins ? "-" : due.length} tone="alert" hint="홍보 인사이트 D+2 · 7 · 14" onClick={() => onGo("probe-insights")} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        <Card title="홍보 인사이트" description="인스타 게시물에 들어간 제휴 매장 · 사장님 보고글" actions={<Button size="sm" variant="ghost" onClick={() => onGo("probe-insights")}>열기</Button>}>
          {!ins ? <Skeleton rows={3} cols={2} /> : ins.papillon_reachable === false ? <p className="text-[12px] text-red-600">Papillon 기획 목록을 못 읽었습니다.</p> : due.length === 0 ? <p className="text-[13px] text-gray-500">지금 보고할 차례인 게시물이 없습니다.</p> : (
            <ul className="space-y-1.5 text-[13px]">{due.slice(0, 4).map((i, k) => <li key={k} className="flex items-center justify-between gap-2"><span className="text-gray-800 truncate"><b>{i.store}</b> · {i.topic}</span><Chip tone="amber">{i.checkpoint.replace("D", "D+")}</Chip></li>)}</ul>
          )}
        </Card>
