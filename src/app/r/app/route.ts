...
import { buildWeeklyAppReport } from "@/lib/draft/appReport";
...
export async function GET(req: Request) {
  ...
  let built;
  try {
    built = await buildWeeklyAppReport({ end });
  } catch (e) {
  ...
  const html = insertAfterBody(
    ...
    downloadBar({
      ...
      canDownload: true,
      statusLabel: `${built.week.label} · ${weekRangeLabel(built.week.start, built.week.end)}${built.warnings.length ? ` · 주의 ${built.warnings.length}건` : ""}`,
    }),
    APP_BODY_TAG
  );
