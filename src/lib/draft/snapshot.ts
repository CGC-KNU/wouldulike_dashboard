...
export function metricsOf(p: PostPerformance | null): ReportMetric[] {
  ...
  return Object.entries(p.metrics).map(([key, m]) => {
    ...
    return { key, value: m.value, median, p10: c?.p10 ?? null, p90: c?.p90 ?? null, n, window_days: c?.window_days ?? null, hidden, delta_pct: ok ? Math.round(((m.value - (median as number)) / (median as number)) * 100) : null, source: "graph", baskets: m.baskets ?? undefined };
  });
}
