 ...
 * 그래서 구독 서비스의 표준 모양으로 바꿨다 — **개시일은 동의한 날, 기간의 정함은 없음, 해지할 때까지 월 단위로 계속.**
 *
...
export const TERMS_VERSION = "wouldulike.partner.terms.v2";
...
export function articles(): Article[] {
  return [
    ...
    { no: "제2조", title: "계약의 개시와 기간", body: [
      "본 계약은 파트너 매장이 본 약관에 동의한 날(이하 “개시일”)에 성립하며, **기간의 정함이 없다.** 어느 한쪽이 해지할 때까지 월 단위로 계속된다.",
      "회사는 개시일부터 매장 정보·혜택의 등록과 홍보물 제작을 수행한다. 이 준비 기간에는 이용료를 청구하지 아니한다.",
    ...
    { no: "제3조", title: "플랜 및 이용료", body: [
      ...
      "이용료의 정산 단위는 1개월로 한다. 파트너 매장은 매월 이용료를 해당 월의 전월 말일까지 회사가 통지한 계좌로 지급하며, 최초 이용료는 개시일부터 7일 이내에 지급한다. 첫 청구가 시작되는 달은 개별 약정란에 기재한다.",
...
export const CHECKS: Check[] = [
  { id: "min_term", article: "제8조", text: (s) => `최소 이용기간은 개시일부터 1개월입니다. 오늘 동의하시면 ${kdate(s.min_term_to)}까지이며, 그 뒤에는 매월 말일까지 말씀만 하시면 다음 달 1일자로 끝납니다. 위약금은 없습니다.` },
...
export function partyLines(v: PartyValues): { label: string; value: string }[] {
  ...
  return [
    ...
    { label: "개시일", value: kdate(s.starts_on) },
